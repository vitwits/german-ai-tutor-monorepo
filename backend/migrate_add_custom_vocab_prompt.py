#!/usr/bin/env python3
"""
Migration script to add 'add_custom_vocab' prompt to the database.
This prompt is used for translating custom words without lesson context.
"""
...
# Define the prompt
ADD_CUSTOM_VOCAB_PROMPT = """Translate the German word or phrase: "{text}". Context: "{ctx}".  
...
async def migrate():
    """Add the custom vocab prompt to database"""
    ...
    async with async_session() as session:
        ...
        if existing:
            print("✅ Prompt 'add_custom_vocab' already exists in database")
            print(f"   Page: {existing.page}")
            print(f"   ID: {existing.id}")
            return
        
        # Create new prompt
        new_prompt = ModelPrompt(
            name="add_custom_vocab",
            page="words",  # Show on Words tab in admin
            prompt=ADD_CUSTOM_VOCAB_PROMPT
        )
        
        session.add(new_prompt)
        await session.commit()
        
        print("✅ Successfully added 'add_custom_vocab' prompt to database")
        print(f"   Name: {new_prompt.name}")
        print(f"   Page: {new_prompt.page}")
        print(f"   ID: {new_prompt.id}")
        print(f"   Length: {len(ADD_CUSTOM_VOCAB_PROMPT)} characters")
    
    await engine.dispose()

if __name__ == "__main__":
    print("🚀 Migrating 'add_custom_vocab' prompt to database...\n")
    asyncio.run(migrate())
    print("\n✨ Migration complete!")
