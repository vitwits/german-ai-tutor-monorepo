...
pipeline {
    ...
    stages {
        ...
        stage('Parallel Checks') {
            when {
                expression {
                    return env.CURRENT_BRANCH.contains('feature/')
                }
            }
                    ...
                    post {
                        always {
                            recordIssues(
                                ...
                                tools: [
                                    pyLint(
                                        id: 'flake8',
                                        name: 'Flake8',
                                        pattern: 'backend/flake8_report.txt'
                                    )
                                ]
                            )
                            ...
                            junit(
                                ...
                                testResults: 'backend/pytest_report.xml'
                            )
                        }
                    }
                }
                    ...
                    post {
                        always {
                            recordIssues(
                                ...
                                tools: [
                                    checkStyle(
                                        id: 'eslint',
                                        name: 'ESLint',
                                        pattern: 'frontend/eslint_report.xml'
                                    )
                                ]
                            )
                            ...
                            junit(
                                allowEmptyResults: true,
                                keepLongStdio: true,
                                testResults: 'frontend/vitest_report.xml'
                            )
                        }
                    }
                }
        ...
        stage('Release (Tests + Build + Push)') {
            when {
                expression {
                    return env.CURRENT_BRANCH == 'main' || env.CURRENT_BRANCH.endsWith('/main')
                }
            }
                ...
                stage('Final Validation') {
                    ...
                    post {
                        always {
                            junit(
                                allowEmptyResults: true,
                                testResults: '**/pytest_report.xml, **/vitest_report.xml'
                            )
                        }
                    }
                }
                ...
                stage('Build and Push Docker Images') {
                    ...
                    post {
                        always {
                            ...
                            sh """
                                docker rmi ${NEXUS_REGISTRY}/german-tutor-backend:${APP_VERSION} \
                                ${NEXUS_REGISTRY}/german-tutor-backend:latest || true
                            """
