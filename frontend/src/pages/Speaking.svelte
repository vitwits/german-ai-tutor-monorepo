<script>
  ...
  $: sourceText = sentence
    ? userLang === "ukr"
      ? sentence.text_uk || sentence.text_en
      : sentence.text_en
    : "";
  $: sourceAudio = sentence
    ? userLang === "ukr"
      ? sentence.audio_uk || sentence.audio_en
      : sentence.audio_en
    : "";
      ...
      <svg viewBox="0 0 140 140">
        ...
        <circle
          ...
          style="stroke-dashoffset: {377 -
            ($scoreAvg / 100) * 377}; stroke: {$scoreAvg >= 80
            ? '#4CAF50'
            : $scoreAvg >= 50
              ? '#FFC107'
              : '#F44336'};"
    ...
    <div class="stats-container">
      <div class="stat-row">
        <div class="stat-label">
          <span>{ui.score_pronunciation}</span> <b>{$scorePron.toFixed(0)}</b>
        </div>
        <div class="progress-bg">
          <div
            class="progress-fill"
            style="width: {$scorePron}%; background: #42A5F5;"
          ></div>
        </div>
      </div>
      <div class="stat-row">
        <div class="stat-label">
          <span>{ui.score_context}</span> <b>{$scoreCont.toFixed(0)}</b>
        </div>
        <div class="progress-bg">
          <div
            class="progress-fill"
            style="width: {$scoreCont}%; background: #66BB6A;"
          ></div>
        </div>
      </div>
      <div class="stat-row">
        <div class="stat-label">
          <span>{ui.score_grammar}</span> <b>{$scoreGram.toFixed(0)}</b>
        </div>
        <div class="progress-bg">
          <div
            class="progress-fill"
            style="width: {$scoreGram}%; background: #FFA726;"
          ></div>
        </div>
      </div>
    </div>
